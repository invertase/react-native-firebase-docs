...
| Parameter |         |
| --------- | ------- |
| picture  | **string** <br /> The bitmap to be used as the payload for the BigPicture notification. Can be an image url such as `https://example.com/filename.jpg` or the filename of a mipmap resource in android such as `ic_launcher`. |
| largeIcon  | **string** (optional) <br />Override the large icon when the big notification is shown. Can be an image url such as `https://example.com/filename.jpg` or the filename of a mipmap resource in android such as `ic_launcher`. |
